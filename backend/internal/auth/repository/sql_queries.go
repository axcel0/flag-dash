...
const (
	...
	getUsersQuery = `SELECT users.id, users.email, users.password, user_profiles.first_name, user_profiles.last_name, role_types.name as role_name, role_types.level as role_level
							FROM users INNER JOIN user_profiles ON users.id = user_profiles.user_id 
							INNER JOIN user_roles ON users.id = user_roles.user_id 
							INNER JOIN role_types ON user_roles.role_id = role_types.id WHERE users.email ILIKE '%' || $1 || '%' OFFSET $2 LIMIT $3;`
